...
(function () {
    ...
    // 新增函数：获取选中内容的唯一标识或最近的锚点元素
    const getPositionAnchor = () => {
        ...
        // 如果没有找到 id，使用 xpath 作为备选方案
        const xpath = getXPath(anchorNode);
        return `#xpath=${encodeURIComponent(xpath)}`;
    };
    ...
    // 生成带标题、链接和定位信息的拼接内容
    const getContentWithSource = (content) => {
        const pageTitle = document.title || "未知标题";
        const baseUrl = window.location.href.split('#')[0]; // 移除已有锚点
        const positionAnchor = getPositionAnchor();
        const pageUrl = positionAnchor ? `${baseUrl}${positionAnchor}` : baseUrl;

        const separator = "\n\n" + "-".repeat(40) + "\n\n";
        const sourceInfo = `来源：${pageTitle}\n链接：${pageUrl}`;

        if (isString(content)) {
            return content + separator + sourceInfo;
        } else if (typeof content === 'object') {
            if (content[TEXT_PLAIN]) {
                content[TEXT_PLAIN] = content[TEXT_PLAIN] + separator + sourceInfo;
            }
            if (content[TEXT_HTML]) {
                // HTML 版本使用可点击的链接
                content[TEXT_HTML] = content[TEXT_HTML] +
                    `<p>${separator.replace(/\n/g, '<br>')}来源：${pageTitle}<br>
          链接：<a href="${pageUrl}" target="_blank">${pageUrl}</a></p>`;
            }
            return content;
        }
        return content;
    ...
    const copy = (data) => {
        // 复制前先拼接来源信息
        const contentWithSource = getContentWithSource(data);
        const params = isString(contentWithSource) ? { [TEXT_PLAIN]: contentWithSource } : contentWithSource;
